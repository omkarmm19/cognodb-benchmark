...
-----------------------
Connects to Neo4j via the official Bolt driver.

Supported targets:
  - Neo4j AuraDB Free (neo4j+s://...)
  - Local Neo4j via Docker (bolt://localhost:7687)

If Neo4j is not reachable this runner returns connected=False and the
benchmark orchestrator will skip and record it as "unavailable". No
simulated or fallback results are produced.

Free tier reference spec (AuraDB Free):
  RAM:     1 GB
  Storage: ~200 MB graph
  CPU:     Shared
  Region:  Configurable (match client region for fairness)
"""
...
class Neo4jRunner(BaseGraphRunner):
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.uri = self.config.get("NEO4J_URI") or os.getenv("NEO4J_URI", "")
        self.user = self.config.get("NEO4J_USER") or os.getenv("NEO4J_USER", "neo4j")
        self.password = self.config.get("NEO4J_PASSWORD") or os.getenv("NEO4J_PASSWORD", "")
        self.driver = None

    def connect(self) -> bool:
        if not self.uri or not self.password:
            print(f"[{self.name}] NEO4J_URI or NEO4J_PASSWORD not set in .env — skipping.")
            return False
        try:
    ...
    def get_footprint(self) -> Dict[str, Any]:
        return {
            "deployment":       "Cloud — Neo4j AuraDB Free tier",
            "vCPU":             "Shared (free tier; exact allocation not published by Neo4j)",
            "RAM_allocated_MB": "Not observable (managed cloud; free tier ~ 1 GB heap stated in docs)",
            "storage":          "Not directly observable (managed cloud)",
            "region":           "Configurable at signup (match CognoDB region for fairness)",
            "memory_usage":     "Not observable",
            "stored_data_size": "Observable in AuraDB console",
        }
